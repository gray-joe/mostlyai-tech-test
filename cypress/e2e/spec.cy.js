describe('Given I am on the homepage', () => {
  beforeEach(() => {
    cy.setCookie('borlabs-cookie', '%7B%22consents%22%3A%7B%22essential%22%3A%5B%22borlabs-cookie%22%2C%22google-tag-manager%22%2C%22mixpanel%22%5D%7D%2C%22domainPath%22%3A%22mostly.ai%2F%22%2C%22expires%22%3A%22Tue%2C%2006%20Feb%202024%2015%3A29%3A23%20GMT%22%2C%22uid%22%3A%22anonymous%22%2C%22version%22%3A%227%22%7D');
  ...
  it('then I should be able to search for results via the search bar, after clicking the search icon', () => {
    ...
    cy.get('.oxy-header-search_form').should('be.visible').within(() => {
    ...
    cy.get('.oxy-header-search_toggle.oxy-header-search_toggle-open').click()
    cy.get('.oxy-header-search_form').should('be.visible').within(() => {
      cy.get('.oxy-header-search_search-field').clear().type('synthetic {enter}')
    });
  ...
  it('and I navigate to the Contact page, then I should be able to fill out the form', () => {
    cy.get('.oxy-mega-dropdown_inner.oxy-header-container')
      .invoke('attr', 'class', 'oxy-mega-dropdown_inner oxy-header-container oxy-mega-dropdown_inner-open')
      .then(() => {
        cy.get('.ct-link.menu-item-area').contains('Contact').click();
      })
    cy.get('#firstname-27854f4c-b840-41f2-b7af-a3c9f512c9e8', { timeout: 30000 }).type('First Name')
    ...
    cy.get('#mobilephone-27854f4c-b840-41f2-b7af-a3c9f512c9e8').type('First Name')
    cy.get('#company-27854f4c-b840-41f2-b7af-a3c9f512c9e8').type('First Name')
    cy.get('#country-27854f4c-b840-41f2-b7af-a3c9f512c9e8').select('United Kingdom')
    cy.get('#how_did_you_hear_about_mostly_ai___free_text-27854f4c-b840-41f2-b7af-a3c9f512c9e8').type('First Name')
